...
def sample_live_icm(g, num_graphs, max_step=2):
    '''
    ...
    # '''
    #     Returns num_graphs live edge graphs sampled from the ICM on g with an optional
    #     limit on the number of propagation steps.
    #     '''
    # live_edge_graphs = []
    # for _ in range(num_graphs):
    #     h = nx.Graph()
    ...
    #         visited = set()
    #         while queue:
    #             current_node, current_step = queue.pop(0)
    #             if current_node in visited:
    #                 continue
    #             visited.add(current_node)
    #             if current_step < max_step:
    #                 for v in g.neighbors(current_node):
    #                     if random.random() < g[current_node][v]['p']:
    #                         h.add_edge(current_node, v)
    #                         queue.append((v, current_step + 1))
    #     live_edge_graphs.append(h)
    # return live_edge_graphs
